...
function HomePage() {
  ...
  // Initialize certificates from localStorage.
  useEffect(() => {
    const savedCerts = JSON.parse(localStorage.getItem('certificates')) || [];
    dispatch({ type: 'INITIALIZE', payload: savedCerts });
  }, []);
  ...
  const handleCreate = () => {
    ...
    // Pass certificate data to the designer page via state.
    navigate(`/designer/${newCert.id}`, { state: { certificate: newCert } });
  };

  // Edit certificate.
  const handleEdit = (id) => {
    const cert = certificates.find((c) => c.id === id);
    // Pass certificate data via navigation state.
    navigate(`/designer/${id}`, { state: { certificate: cert } });
  };
  ...
  const confirmRename = () => {
    const updatedCert = { ...currentCertToRename, name: newName };
    const updatedCerts = certificates.map((cert) =>
      cert.id === currentCertToRename.id ? updatedCert : cert
    );
    localStorage.setItem('certificates', JSON.stringify(updatedCerts));
    dispatch({ type: 'UPDATE_CERTIFICATE', payload: updatedCert });
    setRenameDialogOpen(false);
  };
